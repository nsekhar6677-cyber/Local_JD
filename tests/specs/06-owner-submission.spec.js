...
test('all three missing: error lists amount, mode and screenshot; nothing submitted', async ({ page, request }) => {
  ...
  await expect(msg).toContainText('Please add the amount you paid');
  await expect(msg).toContainText('the payment mode');
  await expect(msg).toContainText('a screenshot of your payment');
...
test('screenshot missing only: blocked with a screenshot message', async ({ page, request }) => {
  ...
  await page.click('#my-mark-paid-btn');
  await expect(page.locator('#my-submit-msg')).toHaveText('Please add a screenshot of your payment before submitting.');
...
test('mode missing only: blocked with a mode message', async ({ page, request }) => {
  ...
  await page.click('#my-mark-paid-btn');
  await expect(page.locator('#my-submit-msg')).toContainText('Please add the payment mode');
  await expect(page.locator('#my-submit-msg')).not.toContainText('screenshot');
  const row = await paidRow(request, 'id3');
  expect(row.paid).toBeFalsy(); // screenshot saved, but not submitted
});

test('amount zero or empty: blocked with an amount message', async ({ page }) => {
  await start(page);
  await ownerUploadShot(page);
  ...
  for (const v of ['0', '']) {
    ...
    await page.click('#my-mark-paid-btn');
    await expect(page.locator('#my-submit-msg')).toHaveText('Please add the amount you paid before submitting.');
  }
});
